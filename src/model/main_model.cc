...
void MainModel::keepSettings()
{
    ...
    QObject::connect(scheduler.get(),
                     SIGNAL(askForPassword(const QString &, bool, int *, const QString &)),
                     this,
                     SIGNAL(askForClientPassword(const QString &, bool, int *, const QString &)));
    QObject::connect(scheduler.get(),
                     SIGNAL(infoSignal(const QString &)),
                     this,
                     SLOT(infoDialog(const QString &)));
    QObject::connect(scheduler.get(),
                     SIGNAL(errorSignal(const QString &)),
                     this,
                     SLOT(errorDialog(const QString &)));

    scheduler->updateExistingTask(settings->getThisApplicationFullPathExecutable(),
                                  CliManager::SCHEDULE_ARGUMENT);
    settings->keepSettings();

    QObject::disconnect(scheduler.get(),
                        SIGNAL(askForPassword(const QString &, bool, int *, const QString &)),
                        this,
                        SIGNAL(askForClientPassword(const QString &, bool, int *, const QString &)));
    QObject::disconnect(scheduler.get(),
...
{
    std::shared_ptr<AbstractScheduler> scheduler(ToolFactory::getSchedulerImpl());
    scheduler->deleteExistingTask(Settings::getInstance()->getThisApplicationFullPathExecutable(),
                                  CliManager::SCHEDULE_ARGUMENT);
        ...
        if (!ssh->loginWithKey()) {
            ...
            if (!ssh->loginWithKey()) {
                showInformationMessage(
                    tr("Can not login to the server. Please review your settings."));
            } else {
...
void MainModel::backup(const BackupSelectionHash &includeRules, const bool &startInCurrentThread)
{
    ...
    // be uploaded
    QObject::connect(this->fsSnapshot, SIGNAL(sendSnapshotDone(int)), this, SLOT(uploadFiles(int)));

    // Connect the info and error signals to the info and error slot
    QObject::connect(this->fsSnapshot,
                     SIGNAL(infoSignal(const QString &)),
                     this,
                     SLOT(infoSlot(const QString &)));
...
void MainModel::schedule(const BackupSelectionHash &includeRules, const ScheduledTask &scheduleRule)
{
    ...
    if (initConnection()) {
        ...
        closeProgressDialogSlot();
        emit showCriticalMessageBox(
            tr("Task has not been scheduled, because the connection could not be established"));
        return;
    }
    Settings *settings = Settings::getInstance();
    ...
    try {
        ...
        case ScheduleRule::AT_WEEKDAYS_AND_TIME:
            scheduler->scheduleTask(settings->getThisApplicationFullPathExecutable(),
                                    CliManager::SCHEDULE_ARGUMENT,
                                    scheduleRule.getTimeToRun(),
                                    scheduleRule.getWeekdaysArray().data());
            break;
        case ScheduleRule::AFTER_BOOT:
            scheduler->scheduleTask(execName,
                                    CliManager::SCHEDULE_ARGUMENT,
                                    scheduleRule.getMinutesAfterStartup());
            break;
...
QList<RestoreName> MainModel::getRestoreNames(const QString &backup_prefix)
{
    ...
    try {
        Settings *settings = Settings::getInstance();
        std::shared_ptr<AbstractRsync> rsync(ToolFactory::getRsyncImpl());
        QStringList restoreInfoFiles
            = rsync->downloadAllRestoreInfoFiles(settings->getApplicationDataDir(), backup_prefix);
        foreach (QString restoreInfoFile, restoreInfoFiles) {
            ...
            if (file.open(QIODevice::ReadOnly)) {
                ...
                if (dateTimeString == MainModel::BACKUP_TIME_TODAY) {
                    ...
                    date = QDateTime::fromString(dateTimeString, MainModel::BACKUP_TIME_FORMAT)
                               .date();
                }

                // extract path to backup (e.g. /.snapthots/daily.0 or /incoming )
                QString backupName = restoreInfoFile;
                backupName = backupName.mid(settings->getApplicationDataDir().length(),
                                            backupName.lastIndexOf(backup_prefix) - 1
                                                - settings->getApplicationDataDir().length());
...
QStringList MainModel::getRestoreContent(const QString &backup_prefix, const QString &backupName)
{
    ...
    try {
        Settings *settings = Settings::getInstance();
        std::shared_ptr<AbstractRsync> rsync(ToolFactory::getRsyncImpl());
        QFileInfo backupContentFileName
        ...
        if (!backupContentFile.open(QIODevice::ReadOnly)) {
            qWarning() << "Can not read backup content file "
...
void MainModel::fullRestore(const QString &backup_prefix,
                            ...
                            const QString &destination)
{
    ...
    RestoreThread *restoreThread = new RestoreThread(backup_prefix,
                                                     backupName,
                                                     selectionRules,
                                                     destination);

    QObject::connect(restoreThread,
    ...
    QObject::connect(restoreThread,
                     ...
                     this,
                     SIGNAL(finishProgressDialog()));
    QObject::connect(this, SIGNAL(abortProcess()), restoreThread, SLOT(abortRestoreProcess()));
    restoreThread->start();
    // TODO: disconnect signal/slot connections
}

void MainModel::customRestore(const BackupSelectionHash &selectionRules,
...
RemoteDirModel *MainModel::loadRemoteDirModel(const QString &backup_prefix,
                                              const QString &backupName)
{
...
void MainModel::uploadFiles(int result)
{
    ...
    QObject::disconnect(this->fsSnapshot,
                        SIGNAL(infoSignal(const QString &)),
                        this,
                        SLOT(infoSlot(const QString &)));
    QObject::disconnect(this->fsSnapshot,
                        SIGNAL(sendSnapshotDone(int)),
                        this,
                        SLOT(uploadFiles(int)));

    QObject::connect(backupThread, SIGNAL(backupFinished()), this, SIGNAL(backupFinished()));
    QObject::connect(backupThread,
                     SIGNAL(showCriticalMessageBox(const QString &)),
                     this,
                     SIGNAL(showCriticalMessageBox(const QString &)));
    QObject::connect(backupThread,
    ...
    QObject::connect(backupThread,
                     ...
                     this,
                     SIGNAL(updateOverviewFormLastBackupsInfo()));
    QObject::connect(this, SIGNAL(abortProcess()), backupThread, SLOT(abortBackupProcess()));
