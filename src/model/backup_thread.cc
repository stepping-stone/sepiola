    ...
    , fsSnapshot(snapshot)
{
    ...
    pt.appendTask(TASKNAME_PREPARE_DIRECTORIES,
                  DateTimeUtils::getDateTimeFromSecs(2 /* time for directory-preparation */),
                  3.0);
    pt.appendTask(TASKNAME_ESTIMATE_BACKUP_SIZE,
                  DateTimeUtils::getDateTimeFromSecs(30 /* time for dry run */),
                  1.0 /* steps??? */);
    pt.appendTask(TASKNAME_FILE_UPLOAD,
                  DateTimeUtils::getDateTimeFromSecs(100),
                  250 /* enter bytes to upload */);
    pt.appendTask(TASKNAME_DOWNLOAD_CURRENT_BACKUP_CONTENT,
                  DateTimeUtils::getDateTimeFromSecs(2 /* backup content download */),
                  3);
    pt.appendTask(TASKNAME_UPLOAD_METADATA,
                  DateTimeUtils::getDateTimeFromSecs(2),
                  1.0 /* steps??? */);
...
BackupThread::~BackupThread()
{
    ...
    QObject::disconnect(rsync.get(),
                        ...
                        this,
                        SLOT(rsyncUploadProgressHandler(const QString &, float, quint64, quint64)));
    QObject::disconnect(this, SIGNAL(abort_rsync()), rsync.get(), SLOT(abort()));
...
void BackupThread::run()
{
    ...
    try {
        ...
        if ((subPt = this->pt.getSubtask(TASKNAME_ESTIMATE_BACKUP_SIZE)) != 0)
            subPt->setTerminated(true);
        ...
        foreach (FilesystemSnapshotPathMapper mapper, this->fsSnapshot->getSnapshotPathMappers()) {
            ...
            relativeProcessedItems = rsync->upload(tmp_includes,
                                                   tmp_source,
                                                   tmp_destination,
                                                   setDeleteFlag,
                                                   compressedUpload,
        ...
        checkAbortState();
        if ((subPt = this->pt.getSubtask(TASKNAME_FILE_UPLOAD)) != 0)
            subPt->setTerminated(true);
        ...
        if (processedItems.size() > 0) {
            ...
            FileSystemUtils::removeFile(settings->getApplicationDataDir()
                                        + settings->getBackupContentFileName());

            emit infoSignal(tr("Getting backup content meta data"));
            QFileInfo currentBackupContentFile
                = rsync->downloadCurrentBackupContentFile(settings->getApplicationDataDir(), false);
            checkAbortState();
            if ((subPt = this->pt.getSubtask(TASKNAME_DOWNLOAD_CURRENT_BACKUP_CONTENT)) != 0)
                subPt->setTerminated(true);
            ...
            checkAbortState();
            if ((subPt = this->pt.getSubtask(TASKNAME_UPLOAD_METADATA)) != 0)
                subPt->setTerminated(true);
            ...
            // permission file
            FileSystemUtils::removeFile(settings->getApplicationDataDir()
                                        + settings->getMetadataFileName());
            FileSystemUtils::removeFile(settings->getApplicationDataDir()
                                        + settings->getTempMetadataFileName());
            ...
            QObject::connect(metadata.get(),
                             ...
                             this,
                             SIGNAL(errorSignal(const QString &)));
            QFileInfo currentMetadataFileName
                = rsync->downloadCurrentMetadata(settings->getApplicationDataDir(), false);
            checkAbortState();

            QFileInfo newMetadataFileName
                = metadata->getMetadata(settings->getApplicationDataDir()
                                            + settings->getTempMetadataFileName(),
                                        processedItems,
                                        this->fsSnapshot,
                                        &warning);
            ...
            if (!warning.isEmpty()) {
                warnings.append("\n");
                warnings.append(
                    tr("Warning: Could not backup file permissions due to following problem:"));
                warnings.append("\n");
                warnings.append(warning);
            }
            checkAbortState();
            if ((subPt = this->pt.getSubtask(TASKNAME_METAINFO)) != 0) {
                subPt->addFixpointNow(subPt->getNumberOfSteps());
                subPt->setTerminated(true);
            }
    ...
    if (failed) {
        ...
        qDebug() << "BackupThread::run()"
                 << "finalStatusSignal( ConstUtils::STATUS_ERROR )";
    } else if (!warnings.isEmpty()) {
        this->setLastBackupState(ConstUtils::STATUS_WARNING);
        qDebug() << "BackupThread::run()"
                 << "finalStatusSignal( ConstUtils::STATUS_WARNING )";
    } else {
        this->setLastBackupState(ConstUtils::STATUS_OK);
        qDebug() << "BackupThread::run()"
                 << "finalStatusSignal( ConstUtils::STATUS_OK )";
    }
...
void BackupThread::setLastBackupState(ConstUtils::StatusEnum newStatus)
{
    ...
    if (this->backupStartDateTime.isNull()) // new Backup
    {
        ...
        this->backupCurrentStatus = (ConstUtils::StatusEnum)
            std::max((int) newStatus, (int) (this->backupCurrentStatus));
    }
    QList<BackupTask> savedBkupInfos = settings->getLastBackups();
    if (!savedBkupInfos.empty()
        && savedBkupInfos.first().getDateTime() == this->backupStartDateTime) {
        this->backupStartDateTime = QDateTime::currentDateTime();
...
void BackupThread::rsyncDryrunProgressHandler(const QString &filename,
                                              long files_total,
                                              long files_done)
{
    ProgressTask *mySubPt = this->pt.getSubtask(
        TASKNAME_ESTIMATE_BACKUP_SIZE); // this->pt.getCurrentTask();
    ...
    if ((mySubPt != 0) && (mySubPt->getName() == TASKNAME_ESTIMATE_BACKUP_SIZE)) {
        ...
        vars.append(
            QPair<QString, QString>(tr("current file"),
                                    StringUtils::filenameShrink(QDir::toNativeSeparators(filename),
                                                                72)));
        vars.append(QPair<QString, QString>(tr("files processed"),
                                            QString("%1 / %2").arg(QString::number(files_done),
                                                                   QString::number(files_total))));
        vars.append(QPair<QString, QString>(tr("estimated remaining time") + ":",
                                            mySubPt->getRootTask()->getEstimatedTimeLeftString()));
    }
    updateInformationToDisplay(vars);
}

void BackupThread::rsyncUploadProgressHandler(const QString &filename,
                                              float traffic,
                                              quint64 bytesRead,
                                              quint64 bytesWritten)
{
    ...
    if (currentTaskName == TASKNAME_FILE_UPLOAD || currentTaskName == TASKNAME_UPLOAD_METADATA
        || currentTaskName == TASKNAME_ESTIMATE_BACKUP_SIZE) {
        mySubPt->addFixpointNow(bytesWritten);
        vars.append(QPair<QString, QString>(tr("current task"), mySubPt->getName()));
        vars.append(QPair<QString, QString>(tr("current file"), filename));
        vars.append(
            QPair<QString, QString>(tr("traffic / transferred") + ":",
                                    StringUtils::bytesToReadableStr((double) traffic, "B/s") + " / "
                                        + StringUtils::bytesToReadableStr((double) bytesWritten
                                                                              + bytesRead,
                                                                          "B")));
        vars.append(QPair<QString, QString>(tr("estimated remaining time") + ":",
                                            mySubPt->getRootTask()->getEstimatedTimeLeftString()));
...
{
    ProgressTask *mySubPt = this->pt.getCurrentTask();
    emit progressSignal(QObject::tr(mySubPt->getName().toUtf8().data()),
                        mySubPt->getRootTask()->getFinishedRatio(),
                        mySubPt->getRootTask()->getEstimatedTimeLeft(),
                        vars);
}
...
void BackupThread::prepareServerDirectories()
{
    ...
    emit progressSignal(mySubPt->getName(),
                        mySubPt->getRootTask()->getFinishedRatio(),
                        mySubPt->getRootTask()->getEstimatedTimeLeft(),
                        vars);

    rsync->upload(QStringList(backupPrefixFolder),
                  source,
                  destination,
                  QStringList(),
                  QStringList(),
...
/**
 * estimates the backup size by running rsync with option --only-write-batch ans updates the
 * progressTask's stepNumber to this size.
 */
quint64 BackupThread::estimateBackupSize(const QString &src, const QString &destination)
{
    ...
    QObject::connect(rsync.get(),
                     SIGNAL(volumeCalculationInfoSignal(const QString &, long, long)),
                     this,
                     SLOT(rsyncDryrunProgressHandler(const QString &, long, long)));
    quint64 uploadSize = rsync->calculateUploadTransfer(includeRules,
                                                        src,
                                                        destination,
                                                        false,
                                                        false,
    ...
    if (uploadPt != 0)
        uploadPt->setNumberOfSteps(uploadSize);

    return uploadSize;
}

void BackupThread::updateBackupContentFile(
    const QFileInfo &backupContentFileName,
    const QList<QPair<QString, AbstractRsync::ITEMIZE_CHANGE_TYPE>> &backupList)
{
    qDebug() << "BackupThread::updateBackupContentFile(" << backupContentFileName.absoluteFilePath()
             << ", [backupList] )";
    ...
    if (!backupContentFile.open(QIODevice::WriteOnly)) {
        qWarning() << "Can not write backup content file "
                   << backupContentFileName.absoluteFilePath();
        return;
    }
...
void BackupThread::uploadBackupStartedXML(long id)
{
    ...
    stream.writeAttribute("xsi:schemaLocation",
                          "http://xml.stepping-stone.ch/schema/backup_started backup_started.xsd");
    stream.writeTextElement("startdate",
                            localTime.toString("yyyy-MM-ddTHH:mm:ss")
                                .append(getTimezoneOffset(time, localTime)));
...
void BackupThread::uploadBackupEndedXML(long id, int success)
{
    ...
    stream.writeAttribute("xmlns:xsi", "http://www.w3.org/2001/XMLSchema-instance");
    stream.writeAttribute("xsi:schemaLocation",
                          "http://xml.stepping-stone.ch/schema/backup_ended backup_ended.xsd");
    stream.writeTextElement("enddate",
                            localTime.toString("yyyy-MM-ddTHH:mm:ss")
                                .append(getTimezoneOffset(time, localTime)));
    stream.writeTextElement("id", QString::number(id, 'f', 0));
...
void BackupThread::uploadSchedulerXML(ScheduledTask schedule)
{
    ...
    stream.writeAttribute(
        "xsi:schemaLocation",
        "http://xml.stepping-stone.ch/schema/online_backup_schedule online_backup_schedule.xsd");
    ...
    case ScheduleRule::AFTER_BOOT:
        ...
        stream.writeTextElement("minutes_after_startup",
                                QString("%1").arg(schedule.getMinutesAfterStartup()));
        stream.writeEndElement();
        break;
    case ScheduleRule::AT_WEEKDAYS_AND_TIME:
        stream.writeStartElement("custom_online_backup");
        stream.writeStartElement("minutes");
        stream.writeTextElement("minute_selected",
                                QString("%1").arg(schedule.getTimeToRun().minute()));
        stream.writeEndElement();
