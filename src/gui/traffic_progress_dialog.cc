...
{
    qDebug() << "TrafficProgressDialog::on_btnShowHideDetails_clicked(): info- and error-textfield "
                "visibility";
...
void TrafficProgressDialog::showFinalStatus(ConstUtils::StatusEnum status)
{
    ...
    this->label_finalStatus->setText(html_templ.arg(statusText,
                                                    rNr.setNum(text_color.red()),
                                                    gNr.setNum(text_color.green()),
                                                    bNr.setNum(text_color.blue())));
    this->label_finalStatus->setStyleSheet(style_templ.arg(rNr.setNum(text_color.red()),
                                                           gNr.setNum(text_color.green()),
                                                           bNr.setNum(text_color.blue())));
}
...
void TrafficProgressDialog::setInfo(QLabel *lbl_label,
                                    QLabel *lpl_value,
                                    QPair<QString, QString> label_and_value)
{
...
void TrafficProgressDialog::setInfoLine(QLabel *lbl_lbl,
                                        QLabel *val_lbl,
                                        QPair<QString, QString> label_and_value)
{
...
void TrafficProgressDialog::updateProgress(const QString & /*taskText*/,
                                           float percentFinished,
                                           const QDateTime & /*timeRemaining*/,
                                           StringPairList infos)
{
    if (!Settings::SHOW_PROGRESS)
        return;

    // qDebug() << "TrafficProgressDialog::updateProgress(" << "," << taskText << "," <<
    // percentFinished << "," << timeRemaining << "," << infos << ")";
    ...
    if (this->lastUpdateTextInfo.addMSecs(msecs_to_wait_for_flush) <= currentTime) {
        // update, but only after a second, such that following info arriving in the next second are
        // also flushed
