...
OverviewForm::OverviewForm(QWidget *parent, MainModel *model) : QWidget(parent)
{
	...
	COLOR_SNAPSHOT = qRgba(255, 127, 0, 255);
	...
	this->refreshLastBackupsOverview(3);
...
void OverviewForm::refreshLastBackupsOverview(int n)
{
	...
	QMap<ScheduledTask::StatusEnum, QPixmap> status_pixmap;
	img.load(":/main/sign_ok.svg");
	status_pixmap.insert(ScheduledTask::OK, QPixmap::fromImage(img));
	img.load(":/main/sign_error.svg");
	status_pixmap.insert(ScheduledTask::ERROR, QPixmap::fromImage(img));
	img.load(":/main/sign_warning.svg");
	status_pixmap.insert(ScheduledTask::WARNING, QPixmap::fromImage(img));
	// fill status-content of last backups into form
	for (int i = 0; i < MAX_LAST_BACKUP_INFOS; i++)
	{
		ScheduledTask::StatusEnum status = (ScheduledTask::StatusEnum)(i%3); // this line will be replaced by the array of last backups
		bool isHidden = i >= n;
		...
		lab_status->setText(ScheduledTask::getStatusText(status));
		lab_date->setHidden(isHidden);
		lab_date->setText((QTime::currentTime()).toString());
	}
