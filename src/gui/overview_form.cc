...
void OverviewForm::refreshLastBackupsOverview()
{
    ...
    fieldnames_status << "labelIconStatusLastBackup_%1"
                      << "labelStatusLastBackup_%1"
                      << "labelDayLastBackup_%1"
                      << "labelDateLastBackup_%1"
                      << "labelTimeLastBackup_%1";
    QImage img;
...
void OverviewForm::refreshSpaceStatistic()
{
    ...
    if (quotaValues.size() == 3) {
        ...
        this->labelLegendBackup->setPixmap(
            this->stackedBarView->legendIcon(spaceUsageModel->index(SpaceUsageModel::BACKUP, 0)));
        this->labelLegendSnapshot->setPixmap(this->stackedBarView->legendIcon(
            spaceUsageModel->index(SpaceUsageModel::INCREMENTAL, 0)));
        this->labelLegendFree->setPixmap(
            this->stackedBarView->legendIcon(spaceUsageModel->index(SpaceUsageModel::FREE, 0)));

        QList<float> sizes;
        sizes << backup << snapshot << (quota - backup - snapshot) << quota;
        ...
        for (int i = 0; i < sizeNames.size(); i++) {
            ...
            QLabel *el_percent = this->findChild<QLabel *>(QString("labelSpacePercent")
                                                           + sizeNames[i]);
            if (el_abs != 0) {
            ...
            if (el_percent != 0) {
                el_percent->setText(
                    format_percent.arg(100.0f * sizes[i] / sizes[sizes.size() - 1], 0, 'f', 1));
            }
        }
    } else {
