...
SettingsForm::SettingsForm(QWidget *parent, MainModel *model)
    : QWidget(parent)
{
    setupUi(this);
    this->model = model;
    ...
    if (quotaValues.size() > 0)
        this->labelQuotaInformation->setText(
            QObject::tr("Total size: %1 GB, free: %2 GB")
                .arg(quotaValues[0] / 1024.0, 0, 'f', 1)
                .arg((quotaValues[0] - quotaValues[2] - quotaValues[2]) / 1024.0, 0, 'f', 1));
    else
        this->labelQuotaInformation->setText(
            QObject::tr("Total size: %1 GB, free: %2 GB").arg("-").arg("-"));
    if (settings->getQuotaModificationUrl() != "") {
        this->labelChangeQuotaLink->setText(
            QObject::tr("<a href=\"%1\">Change quota</a> (opens a browser window)")
                .arg(settings->getQuotaModificationUrl().replace(uid_param,
...
void SettingsForm::reload()
{
    ...
    this->comboBoxLanguage->setCurrentIndex(
        this->comboBoxLanguage->findData(settings->getLanguage()));
    onFormChange();
}
...
void SettingsForm::save()
{
    ...
    settings->saveLanguage(
        this->comboBoxLanguage->itemData(this->comboBoxLanguage->currentIndex()).toString());
...
void SettingsForm::reset()
{
    ...
    int answer = QMessageBox::warning(this,
                                      ...
                                      tr("Your settings have been modified.\n"
                                         "Do you want to restore your previous settings?"),
...
{
    Settings *settings = Settings::getInstance();
    if (this->lineEditUsername->text() != settings->getServerUserName()
        ...
        || this->comboBoxLanguage->currentIndex()
               != this->comboBoxLanguage->findData(settings->getLanguage())) {
        formChanged = true;
    } else {
        formChanged = false;
    ...
    QList<QAbstractButton *> buttons = this->buttonBox->buttons();
    foreach (QAbstractButton *button, buttons) {
        if (this->buttonBox->buttonRole(button) == QDialogButtonBox::AcceptRole
            || this->buttonBox->buttonRole(button) == QDialogButtonBox::RejectRole) {
