...
{
    QObject::disconnect(this->comboBoxPrefixes,
                        SIGNAL(currentIndexChanged(int)),
                        this,
                        SLOT(initRestoreNames()));
}

RestoreForm::RestoreForm(QWidget *parent, MainModel *model)
    : QWidget(parent)
{
    setupUi(this);
    initialized = false;
    ...
    QObject::connect(this->comboBoxPrefixes,
                     SIGNAL(currentIndexChanged(int)),
                     this,
                     SLOT(initRestoreNames()));
}
...
{
    qDebug() << "RestoreForm::initRestoreNames()"
             << "prefix:" << this->comboBoxPrefixes->currentText();
    this->model->showProgressDialogSlot(tr("Downloading restore meta data"));
    QObject::disconnect(this->comboBoxBackupNames,
                        SIGNAL(currentIndexChanged(int)),
                        this,
                        SLOT(populateFilesAndFoldersTree()));
    ...
    QList<RestoreName> restoreNames = this->model->getRestoreNames(
        this->comboBoxPrefixes->currentText());
    ...
    if (restoreNames.size() > 0) {
        ...
        foreach (RestoreName restoreName, restoreNames) {
            if (restoreName.getDate() == QDate::currentDate()) {
                ...
                this->comboBoxBackupNames->addItem(restoreName.getText(),
                                                   restoreName.getAbsoluteDirName());
            }
        }
        populateFilesAndFoldersTree();
        QObject::connect(this->comboBoxBackupNames,
                         SIGNAL(currentIndexChanged(int)),
                         this,
                         SLOT(populateFilesAndFoldersTree()));
    } else {
...
void RestoreForm::setBackupSelectionDisabled(const bool &disable)
{
    ...
    if (disable)
        this->treeViewFilesAndFolders->setModel(0);
}
...
void RestoreForm::refreshRemoteDirModel()
{
    ...
    QString currentBackupName
        = this->comboBoxBackupNames->itemData(this->comboBoxBackupNames->currentIndex()).toString();
    ...
    this->treeViewFilesAndFolders->setModel(
        this->model->loadRemoteDirModel(currentPrefix, currentBackupName));
...
void RestoreForm::startRestore(bool isFullRestore, QString destination)
{
    ...
    if (!this->treeViewFilesAndFolders->selectionModel()) {
        this->model->showInformationMessage(
            tr("You can not start a restore because no backup is available."));
        return;
    }
    ...
    QString selectedBackupName
        = this->comboBoxBackupNames->itemData(this->comboBoxBackupNames->currentIndex()).toString();
    QString selectedBackupPrefix = this->comboBoxPrefixes->currentText();
    if (!isFullRestore
        && (this->model->getCurrentRemoteDirModel()->getSelectionRules().size() == 0)) {
    ...
    if (destination != Platform::ROOT_PREFIX) {
        ...
        while ((destination == "" || !QFileInfo(destination).exists())
               && !(selectionAborted = !browseForDestinationFolder(destination))
               && destination == "") {
            this->model->showInformationMessage(
                "No valid destination folder has been selected.\nPlease select a valid restore "
                "destination.");
        }
        if (selectionAborted) {
            return;
    ...
    if (isFullRestore) {
        ...
        BackupSelectionHash selectionRules = this->model->getCurrentRemoteDirModel()
                                                 ->getSelectionRules();
        this->model->customRestore(selectionRules,
                                   selectedBackupPrefix,
                                   selectedBackupName,
                                   destination);
    }
}
