...
public:
    ...
    ProgressTask(ProgressTask *parent,
                 QString name,
    ...
    void setFixpointsChanged(
        bool fp_changed = true); // used by collectFixpointsFromSubtasks to avoid unnecessary runs
    ...
    inline void debugIsCorrectCurrentTask(const QString &expectedName)
    {
        QString curName = this->getCurrentTask()->getName();
        if (curName.compare(expectedName) != 0) {
            qDebug() << "current task is wrong! current task:" << curName << "expected task"
                     << expectedName << "-> solution: terminate finished tasks!";
        } else {
            qDebug() << "current task is:" << curName;
        }
    };
...
private:
    static const double DURATION_CALCULATION_TIME_RANGE; // only the activities of the last TIME_RANGE
