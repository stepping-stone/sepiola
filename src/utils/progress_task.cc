...
const double ProgressTask::DURATION_CALCULATION_TIME_RANGE
    = 40.0; // only the activities of the last TIME_RANGE seconds are taken into Account
...
ProgressTask::ProgressTask(ProgressTask *parent,
                           QString name,
                           const QDateTime &estDuration,
                           const double nSteps)
    : isFinished(false)
    , fp_changed(true)
    , parent(parent)
...
ProgressTask *ProgressTask::appendTask(const QString &name,
                                       const QDateTime &estDuration,
                                       const double nSteps)
{
...
void ProgressTask::addFixpoint(QDateTime time, double stepN, bool force)
{
    if (this->fp_steps.size() > 0 && stepN < this->fp_steps.last()) {
        ...
        if (force || this->fp_times.size() == 0
            || (DateTimeUtils::getSeconds(time) - DateTimeUtils::getSeconds(this->fp_times.last())
                > 0.2)) {
...
QDateTime ProgressTask::getRunningTime()
{
    if (this->fp_times.size() > 0) {
        return (
            DateTimeUtils::getDateTimeFromSecs(DateTimeUtils::getSeconds(fp_times.last())
                                               - DateTimeUtils::getSeconds(this->getStartTime())));
    } else {
...
/**
 * returns either the estimated total duaration, if there are fixpoints available or the provided
 * initial guess
 */
QDateTime ProgressTask::getTotalDurationBestGuess()
{
    if (fp_steps.size() >= 2) {
        return DateTimeUtils::getDateTimeFromSecs(
            std::max<double>(0.0, DateTimeUtils::getSeconds(this->getEstimatedTotalDuration())));
    } else {
...
QDateTime ProgressTask::getEstimatedTimeLeft()
{
    ...
    if (this->fp_steps.size() < 2) {
        if (fp_times.size() > 0) {
            ...
            return (DateTimeUtils::getDateTimeFromSecs(
                std::max<double>(0.0,
                                 DateTimeUtils::getSeconds(this->getProposedEstimatedDuration())
                                     + DateTimeUtils::getSeconds(fp_times.last())
                                     - DateTimeUtils::getSeconds(QDateTime::currentDateTime()))));
        } else {
        ...
        while (i_from > 0
               && t_last_secs - DateTimeUtils::getSeconds(fp_times[i_from])
                      <= DURATION_CALCULATION_TIME_RANGE)
            i_from--;
        ...
        double remaining_secs_from_last_fp = (this->getNumberOfSteps() - fp_steps.last()) * sum_dxdy
                                             / sum_dx2;
        QDateTime guess = DateTimeUtils::getDateTimeFromSecs(
            remaining_secs_from_last_fp + DateTimeUtils::getSeconds(fp_times.last())
            - DateTimeUtils::getSeconds(QDateTime::currentDateTime()));
        ...
        if (DateTimeUtils::getSeconds(guess) < 2592000.0) {
            // the estimation is fed into a list, then a mean of these guesses (with respect to the
            // time passed) is returned
            this->lastDurationGuesses.prepend(
                QPair<QDateTime, QDateTime>(QDateTime::currentDateTime(), guess));
        }
        double retSecs = 0.0, curSecs = DateTimeUtils::getSeconds(QDateTime::currentDateTime());
        int i = -1;
        while (++i < this->lastDurationGuesses.size()
               && (curSecs - DateTimeUtils::getSeconds(this->lastDurationGuesses[i].first)
                   < this->MEAN_OVER_LAST_N_SECONDS)) {
            retSecs += std::max<double>(0.0,
                                        DateTimeUtils::getSeconds(this->lastDurationGuesses[i].first)
                                            + DateTimeUtils::getSeconds(
                                                this->lastDurationGuesses[i].second)
                                            - curSecs);
        }
...
{
    return DateTimeUtils::getDateTimeFromSecs(
        DateTimeUtils::getSeconds(getEstimatedTimeLeft()) + DateTimeUtils::getSeconds(getStartTime())
        - DateTimeUtils::getSeconds(QDateTime::currentDateTime()));
}
        ...
        for (int i = 0; i <= subPt->getNumberOfSteps(); i++) {
            if (i != 0) {
                ...
                waitCondition.wait(&mutex,
                                   (long) (10
                                           + DateTimeUtils::getSeconds(
                                                 subPt->getProposedEstimatedDuration())
                                                 / subPt->getNumberOfSteps()
                                                 * (task_nr == 1 ? 2 : 1) * (rand() % 1000)));
            }
            subPt->addFixpointNow(i);
            qDebug() << i << "of" << subPt->getNumberOfSteps() << pt.getEstimatedTimeLeftString()
                     << " left" << subPt->getRunningTime().toString("h:mm:ss.zzz");
        }
        subPt->setTerminated();
    }
}
