...
void Schtasks::scheduleTask(const QString &execName,
                            const QString &cliArgument,
                            const int &minutesToDelay)
{
    qDebug() << "Schtasks::setScheduleTask( " << execName << ", " << cliArgument << ", "
             << minutesToDelay << " )";
...
void Schtasks::scheduleTask(const QString &execName,
                            ...
                            const bool days[])
{
    qDebug() << "Schtasks::setScheduleTask( " << execName << ", " << cliArgument << ", " << time
             << ", days )";
    Settings *settings = Settings::getInstance();
    QLocale locale = getSchtasksBinaryLocale();
    QString dayString;
...
bool Schtasks::schedule(const QString &execName,
                        const QString &cliArgument,
                        const QStringList &schtasksArguments,
                        const QLocale &locale)
{
    qDebug() << "Schtasks::schedule( " << execName << ", " << cliArgument << ", "
             << schtasksArguments << ", " << locale << " )";

    QString username = QString::fromWCharArray(_wgetenv(L"USERDOMAIN")) + "\\"
                       + QString::fromWCharArray(_wgetenv(L"USERNAME"));

    QString password;
    bool schtasks_success = false;
    bool password_ok
        = getWindowsPassword(username,
                             password,
                             QObject::tr("Enter login information to create the schedule task."));
    ...
    if (password_ok) {
        ...
        QTextCodec *codec = 0;
        if ((codec = QTextCodec::codecForName(codecName.toLatin1())) != 0
            || (codec = QTextCodec::codecForName(("CP_" + codecName).toLatin1())) != 0) {
    ...
    if (schtasks_success) {
        ...
        emit infoSignal(
            QObject::tr("Scheduled task could not be created\nPlease make sure you have enough "
                        "permission. Restart the application by right-clicking on the application "
                        "icon, select \"Run as administrator\" and try again."));
    }
...
bool Schtasks::updateExistingTask(const QString &execName, const QString &cliArgument)
{
    ...
    if (taskExists()) {
        ...
        QString username = QString::fromWCharArray(_wgetenv(L"USERDOMAIN")) + "\\"
                           + QString::fromWCharArray(_wgetenv(L"USERNAME"));
        QString password;
        bool password_ok = getWindowsPassword(
            username,
            password,
            QObject::tr("Enter login information to update the existing schedule task."));
        ...
        if (password_ok) {
            ...
            QString codecName = getCodecName();
            QTextCodec *codec = 0;
            if ((codec = QTextCodec::codecForName(("CP" + codecName).toLatin1())) != 0
                || (codec = QTextCodec::codecForName(codecName.toLatin1())) != 0) {
                lineData = codec->toUnicode(readAll());
            } else {
                lineData = QString::fromLocal8Bit(readAll());
            }
            if (lineData.contains(getTranslationForWARNING(locale))) {
                schtasks_success = false;
                QString warning = lineData.right(
                    lineData.size() - lineData.indexOf(getTranslationForWARNING(locale)));
                qDebug() << warning;
                LogFileUtils::getInstance()->writeLog(warning);
            }
            if (lineData.contains(getTranslationForERROR(locale))) {
...
QString Schtasks::getCodecName()
{
    ...
    GetLocaleInfo(LOCALE_USER_DEFAULT,
                  ...
                  oemCodePage,
                  sizeof(oemCodePage) / sizeof(char));
...
QString Schtasks::getTranslationForWrongAccountInformation(const QLocale &locale)
{
    ...
    case QLocale::German:
        return QString::fromWCharArray(
            L"wurde erstellt, aber er kann nicht ausgef\u00FChrt werden, weil die "
            L"Kontoinformationen nicht festgelegt werden konnten.");
    default:
        break;
    }
...
void Schtasks::testSchtasksScheduleTask()
{
    ...
    try {
        schtasks.scheduleTask("c:\\program files\\sepiola 0.3.0\\bin\\sepiola.exe",
...
namespace {
int dummy1 = TestManager::registerTest("testSchtasksScheduleTask",
                                       Schtasks::testSchtasksScheduleTask);
}
