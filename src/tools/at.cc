...
void At::scheduleTask(const QString& execName, const QString& cliArgument, const QTime& time, const bool days[])
{
	qDebug() << "At::scheduleTask( " << execName << ", " << cliArgument << ", " << time << ", days )";
			...
			if (entry.contains(Settings::getInstance()->getThisApplicationExecutable()) )
			{
				...
				for (int i=0; i<propertyEntries.size(); i++)
				{
					if (propertyEntries.at(i).startsWith("Schedule:"))
					{
						...
						days_entry = prop_entry.mid(prop_entry.indexOf("Each ")+5).trimmed().replace(" ",",");
					}
					if (propertyEntries.at(i).startsWith("Time of day:"))
					{
						QString prop_entry = propertyEntries.at(i);
						time_entry = prop_entry.mid(prop_entry.indexOf("Time of day")+12).trimmed().replace(" ","");
					}
				}
				// AT outputs "22:00 PM" on some systems, but doesn't accept "22:00PM" as input, delete "PM" in cases where hour >= 12
