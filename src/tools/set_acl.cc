...
void SetAcl::mergeMetadata(
    const QFileInfo &newMetadataFileName,
    const QFileInfo &currentMetadataFileName,
    const QList<QPair<QString, AbstractRsync::ITEMIZE_CHANGE_TYPE>> &processedItems)
{
    // Convert the processedItem-path to the correspondign operating system: E.g. windows:
    ...
    qDebug() << "SetAcl::mergeMetadata( " << newMetadataFileName.absoluteFilePath() << ", "
...
void SetAcl::setMetadata(const QFileInfo &metadataFileName,
                         const QStringList &downloadedItems,
                         const QString &downloadDestination)
{
    qDebug() << "SetAcl::setMetadata( " << metadataFileName.absoluteFilePath() << ", ..., "
             << downloadDestination << " )";
    ...
    foreach (QString item, downloadedItems) {
        ...
        if (downloadDestination == "/cygdrive") {
            ...
            aclValue = ITEM_NAME_PREFIX + absoluteItemPath
                       + aclValue.right(aclValue.size() - item.size() - ITEM_NAME_PREFIX_SIZE);
    ...
    arguments << "-ot"
              ...
              << "-actn"
              << "restore"
              << "-bckp" << metadataFileName.absoluteFilePath();
...
void SetAcl::writeMapContentToFile(const QMap<QString, QString> &aclMap,
...
void SetAcl::replaceUncPathsByLocalPathsInMetadataFile(const QFileInfo &Filename,
