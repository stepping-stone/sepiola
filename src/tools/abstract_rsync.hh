...
public:
	 ...
	 * Transfer type
	 */
	enum ITEMIZE_CHANGE_TYPE { UPLOADED, DOWNLOADED, SKIPPED, DELETED };
	 ...
	 */
	virtual QList< QPair<QString, AbstractRsync::ITEMIZE_CHANGE_TYPE> > upload( const QStringList& items, const QString& source, const QString& destination, const QStringList& includePatternList, const QStringList& excludePatternList, bool setDeleteFlag, bool compress, QString* errors ) = 0;
	 ...
	 * @return the content file
	 */
	virtual QFileInfo downloadCurrentBackupContentFile( const QString& destination, bool emitErrorSignal = true ) = 0;
...
inline void AbstractRsync::upload( const QFileInfo& fileInfo, const QString& destination, bool compress, QString* errors )
{
	QStringList fileNameList;
	fileNameList << fileInfo.fileName();
	upload(fileNameList, fileInfo.absolutePath(), destination, QStringList(), QStringList(), false, compress, errors );
}
