...
void Crontab::scheduleTask(const QString &execName,
                           const QString &cliArgument,
                           const int &minutesToDelay)
{
    qDebug() << "Crontab::setScheduleTask( " << execName << ", " << cliArgument << ", "
             << minutesToDelay << " )";
...
void Crontab::scheduleTask(const QString &execName,
                           ...
                           const bool days[])
{
    qDebug() << "Crontab::setScheduleTask( " << execName << ", " << cliArgument << ", " << time
             << ", days )";
...
bool Crontab::writeCrontabEntries_Helper(QStringList &crontabEntries, QString &return_error)
{
    ...
    if (!tmpFile.open()) {
        return_error = QObject::tr("Scheduling failed, because file %1 could not be written")
...
bool Crontab::writeAllCrontabEntries(QStringList &crontabEntries)
{
    ...
    if (writeCrontabEntries_Helper(crontabEntries, error)) {
        // under some systems (f.ex. centos) there are additionally returned some comments when
        // calling "crontab -l" these entries must be removed and the rest again inserted
...
QStringList Crontab::removeExistingBackupJob(const QStringList &existingEntries,
...
QStringList Crontab::updateExistingBackupJob(const QStringList &existingEntries,
                                             const QString &execName,
                                             const QString &cliArgument)
{
    qDebug() << "Crontab::updateExistingBackupJob( " << existingEntries << ", " << execName << ", "
             << cliArgument << " )";
...
bool Crontab::schedule(const QString &execName,
                       const QString &cliArgument,
                       const QString &timeSpecification)
{
    static const char *LANG_VARS[] = {"LC_ALL", "LC_CTYPE", "LANG"};

    qDebug() << "Crontab::schedule( " << execName << ", " << cliArgument << ", "
             << timeSpecification << " )";
...
bool Crontab::isVixieCron()
{
    ...
    if (crontabFile.size()
        > 2 * 1024 * 1024) // if the binary's size is too big, assume that this is a wrong file
    {
