...
QList<QPair<QString, AbstractRsync::ITEMIZE_CHANGE_TYPE>> Rsync::upload(
    ...
    bool dry_run)
{
    qDebug() << "Rsync::upload(" << includeRules << "," << src << "," << destination << ","
             << setDeleteFlag << "," << compress << "," << (warnings ? *warnings : "(no warnings)")
             << "," << dry_run << ")";
    ...
    if (dry_run) {
        this->last_calculatedLiteralData = 0;
        arguments << "-v"
                  << "--stats"
                  << "--only-write-batch=/dev/null";
    }
    ...
    foreach (QString include_dir, include_dirs_list) { /* qDebug() << "$$$" << include_dir; */
        convertedIncludeDirs.append(convertFilenameForRsyncArgument(include_dir));
    }
    if (StringUtils::writeQByteArrayListToFile(convertedIncludeDirs,
                                               include_dirs_filename,
                                               Platform::EOL_CHARACTER)) {
    ...
    while (blockingReadLine(&lineData, 2147483647, rsyncEolChar)) // -1 does not work on windows
    {
        ...
        if (lineData.size() > 0) {
            ...
            if (lineData.contains(STATISTICS_FIRST_USED_LABEL)) {
                ...
                this->last_calculatedLiteralData = lineData
                                                       .mid(STATISTICS_FIRST_USED_LABEL.length())
                                                       .trimmed()
                                                       .split(" ")[0]
                                                       .toLong();
            }
            QPair<QString, AbstractRsync::ITEMIZE_CHANGE_TYPE> item
                = getItemAndStoreTransferredBytes(lineData);

            if (dry_run && dryrun_state == DRY_RUN_START
                && lineData.contains("building file list ...")) {
                dryrun_state = DRY_RUN_COUNT_FILES;
            }
            if (dry_run && dryrun_state == DRY_RUN_COUNT_FILES
                && lineData.contains("files to consider")) {
            ...
            if (dry_run && dryrun_state == DRY_RUN_CALCULATE_SIZE) {
                if (!lineData.startsWith("<f") && !lineData.startsWith("cd")) {
                    if (lineData.contains("to-check=")) {
                        QStringList files_cur_total
                            = lineData.trimmed().split("to-check=").last().split("/");
                        this->files_total
                            = nOfFiles; // files_cur_total.last().replace(")","").toLong();
            ...
            if (!dry_run) {
                if (!endReached && item.first != "./") {
                    ...
                    case TRANSFERRED:
                        ...
                        emit trafficInfoSignal(this->progress_lastFilename,
                                               this->progress_trafficB_s,
                                               this->progress_bytesRead,
                                               this->progress_bytesWritten);
                        break;
                    case SKIPPED:
...
void Rsync::updateInfoOutput()
{
    ...
    emit volumeCalculationInfoSignal(this->progress_lastFilename,
                                     this->files_total,
                                     this->cur_n_files_done);
}
void Rsync::bufferedInfoOutput() // prevents direct flush
{
    ...
    if (this->lastUpdateTime.addMSecs(msecs_to_wait_for_flush) <= currentTime) {
        ...
        // update, but only after half a second, such that following info arriving in the next
        // second are also flushed
        this->lastUpdateTime = currentTime;
        // QTimer::singleShot(msecs_to_wait_for_flush, this, SLOT(updateInfoOutput())); //
        // funktioniert unerklaerlicherweise nicht
        updateInfoOutput();
    }
}
    ...
    bool dry_run)
{
    QString STATISTICS_FIRST_USED_LABEL = "Literal data:";
    QString STATISTICS_FIRST_LABEL = "Number of files:";
    qDebug() << "Rsync::upload(" << items << ", " << src << ", " << destination << ", "
             << includePatternList << ", " << excludePatternList << ")";
    Settings *settings = Settings::getInstance();
    ...
    arguments << getRsyncUploadArguments();
    if (dry_run) {
        this->last_calculatedLiteralData = 1;
        arguments << "--stats"
                  << "--only-write-batch=/dev/null";
    }
    ...
    while (blockingReadLine(&lineData, 2147483647)) // -1 does not work on windows
    {
        ...
        if (lineData.contains(STATISTICS_FIRST_USED_LABEL)) {
            endReached = true; // hopefully not necessary
            this->last_calculatedLiteralData = lineData.mid(STATISTICS_FIRST_USED_LABEL.length())
                                                   .trimmed()
                                                   .split(" ")[0]
                                                   .toLong();
        }
        QPair<QString, AbstractRsync::ITEMIZE_CHANGE_TYPE> item = getItemAndStoreTransferredBytes(
            lineData);
        if (!endReached && item.first != "") {
            item.first.prepend("/");
            removeSymlinkString(&item.first);
...
long Rsync::calculateUploadTransfer(const BackupSelectionHash includeRules,
                                    const QString &src,
                                    const QString &destination,
                                    bool setDeleteFlag,
                                    bool compress,
...
QStringList Rsync::downloadFullBackup(const QString &backup_prefix,
...
QStringList Rsync::downloadCustomBackup(const QString &backup_prefix,
                                        const QString &backupName,
...
QFileInfo Rsync::downloadBackupContentFile(const QString &backup_prefix,
                                           const QString &backupName,
                                           const QString &destination)
{
    qDebug() << "Rsync::downloadBackupContentFile(" << backup_prefix << "," << backupName << ","
             << destination << ")";
    Settings *settings = Settings::getInstance();
...
QFileInfo Rsync::downloadCurrentBackupContentFile(const QString &destination, bool emitErrorSignal)
{
    ...
    QString source = settings->getBackupRootFolder() + settings->getBackupPrefix() + "/"
                     + settings->getMetaFolderName() + "/";

    downloadSingleFile(source,
                       destination,
                       settings->getBackupContentFileName(),
                       true,
                       emitErrorSignal);
    return destination + settings->getBackupContentFileName();
}

QFileInfo Rsync::downloadMetadata(const QString &backup_prefix,
                                  const QString &backupName,
                                  const QString &destination)
{
    qDebug() << "Rsync::downloadMetadata(" << backup_prefix << "," << backupName << ","
             << destination << ")";
    Settings *settings = Settings::getInstance();
    QString source = backupName + "/" + backup_prefix + "/" + settings->getMetaFolderName() + "/";

    return downloadSingleFile(source, destination, settings->getMetadataFileName(), true, false);
    // return destination + settings->getMetadataFileName(); // like that, it is not clear, if
    // metadata was successfully downloaded or not
}
...
QFileInfo Rsync::downloadSingleFile(const QString &source,
                                    const QString &destination,
                                    const QFileInfo &fileName,
                                    bool compress,
                                    bool emitErrorSignal)
{
    QStringList downloadedItems = download(source,
                                           destination,
                                           QStringList(fileName.fileName()),
                                           compress,
                                           emitErrorSignal);
    if (downloadedItems.size() != 1) {
        qWarning() << "Download error for file " << fileName.fileName();
        qDebug() << "Downloaded file(s): " << downloadedItems;
        return QFileInfo(""); // destination + "/" + fileName.fileName(); // now returns an empty
                              // QString, if download failed, so that no old metadata gets applied
    }
 ...
 */
QStringList Rsync::download(const QString &source,
                            const QString &destination,
                            ...
                            bool emitErrorSignal)
{
    ...
    if (this->exitCode() != 0) {
        if (emitErrorSignal) {
            throw ProcessException(QObject::tr("rsync exited with with exitCode %1 (%2 %3).")
                                       .arg(this->exitCode())
                                       .arg(errors)
                            ...
                            bool emitErrorSignal)
{
    ...
    if (includeRules.size() > 0) {
        ...
        if (StringUtils::writeQByteArrayListToFile(calculateRsyncRulesFromIncludeRules(includeRules),
                                                   includesFilename,
                                                   QByteArray(Platform::EOL_CHARACTER))) {
                                               ...
                                               const QString &backup_prefix)
{
    ...
    while (blockingReadLine(&lineData, 2147483647)) {
        ...
        QString item = getItemAndStoreTransferredBytes(lineData).first;
        if (!item.contains("=>")
            && item.contains(settings->getMetaFolderName() + "/"
                             + settings->getBackupTimeFileName())) {
...
QStringList Rsync::getPrefixes()
{
    ...
    QString src = settings->getServerUserName() + "@" + settings->getServerName() + ":"
                  + StringUtils::quoteText(settings->getBackupRootFolder(), "'");
    arguments << "--include=/*/";
    arguments << "--exclude=*";
    arguments << src;
...
QStringList Rsync::getRsyncSshArguments()
{
    ...
    argument.append(" -F "
                    + StringUtils::quoteText(settings->getSshConfigDataDir() + "config", "'"));
    ...
    argument.append(" -o "
                    + StringUtils::quoteText("UserKnownHostsFile = \""
                                                 + settings->getSshConfigDataDir() + "known_hosts\"",
                                             "'"));
    argument.append(" -o " + StringUtils::quoteText("PreferredAuthentications publickey", "'"));
    // ignore any running SSH agents since they may offer additional keys first, causing
                                                             ...
                                                             QStringList *files_from_list)
{
    ...
    if (files_from_list != 0) {
        files_from_list->clear();
        files_from_list->append("/");
    } // clear list and add "/"
    foreach (QString rule, includeRulesList) {
        bool ruleMod = includeRules[rule];
        QString ruleParentDir = StringUtils::parentDir(rule);
        QString ruleDir = StringUtils::dirPart(rule);
        // qDebug() << "rule" << (ruleMod?"+":"-") << rule << "( ruleDir" << ruleDir << "curDir" <<
        // curDir << ")" << "isSubDir" << isSubDir << "isParentDir" << isParentDir;
        ...
        while (unclosedDirs.size() > 0 && !ruleDir.startsWith(unclosedDirs.top().first)) {
            ...
            if (dirToClose.second
                || dirToClose.first != lastRule) { // don't exclude dir/** bejond dir/
        ...
        while (curDir != ruleParentDir) {
            ...
            if (curDir == StringUtils::dirPart(lastRule)
                && !includeRules[lastRule]) { // remove exclude before identical include
                filters.removeLast();
            }
            filters << convertRuleToByteArray(curDir, true);
            if (files_from_list != 0) {
                files_from_list->append(curDir);
            }
    ...
    if (includeRules.isEmpty()) {
        filters.clear();
    } else if (includeRules[includeRulesList.first()]) { // the toplevel rule distinguishes between
                                                         // include or exclude
...
QPair<QString, AbstractRsync::ITEMIZE_CHANGE_TYPE> Rsync::getItemAndStoreTransferredBytes(
    QString rsyncOutputLine)
{
    if (rsyncOutputLine.startsWith(" ") && rsyncOutputLine.contains("%")
        && rsyncOutputLine.contains("B/s")) {
        QString trafficStr;
        QStringList parts = rsyncOutputLine.simplified().split(" ");
        if (parts.size() >= 1 && parts[parts.size() - 1].trimmed().startsWith("(")) {
            parts.pop_back();
        }
...
void Rsync::processWarningsAndErrors(QString *warnings)
{
    ...
    if (warnings->isEmpty()) {
        ...
        case 23: // Partial transfer due to error
            ...
            *warnings += "\n";
            *warnings += tr(
                "Warning: At least one file could not be backuped up. See above for details.");
            return;
        case 24: // Partial transfer due to vanished source files
            *warnings += standardErrors;
            *warnings += "\n";
            *warnings += tr("Warning: Some files have been renamed, moved or deleted during backup "
                            "before they could be uploaded to the server.\n"
                            "Usually this concerns temporary files and this warning can be "
                            "ignored. See above for details.");
            return;
        }
    } else if (exitCode == 23) {
...
void Rsync::testUpload()
{
    ...
    files << "/tmp2/";
    QString source = "/";
    QString destination = settings->getServerUserName() + "@" + settings->getServerName() + ":"
                          + StringUtils::quoteText(settings->getBackupRootFolder() + backup_prefix
...
void Rsync::testDownloadBackupContentFile()
{
    Rsync rsync;
    QString backup_prefix = Settings::getInstance()->getBackupPrefix();
    rsync.downloadBackupContentFile(backup_prefix,
                                    Settings::getInstance()->getBackupRootFolder(),
...
int dummy2 = TestManager::registerTest("testDownloadCurrentMetadata",
                                       Rsync::testDownloadCurrentMetadata);
int dummy3 = TestManager::registerTest("testDownloadAllRestoreInfoFiles",
                                       Rsync::testDownloadAllRestoreInfoFiles);
int dummy4 = TestManager::registerTest("testDeleteAllRestoreInfoFiles",
                                       Rsync::testDeleteAllRestoreInfoFiles);
int dummy5 = TestManager::registerTest("testDownloadBackupContentFile",
                                       Rsync::testDownloadBackupContentFile);
