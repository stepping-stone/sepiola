...
void Process::start()
{
    ...
    LogFileUtils::getInstance()->writeLog("Started: " + executableName + " "
                                          + log_arguments.join(" "));

    qProcess->start(executableName, arguments);
    if (qProcess->waitForStarted()) {
        qDebug() << "         " << executableName << " started successfully.";
        LogFileUtils::getInstance()->writeLog("         " + executableName
                                              + " started successfully.");
        return;
    }
    qCritical() << "Failed: " << executableName << log_arguments;
    throw ProcessException(
        QObject::tr("The process %1 failed to start. Either the invoked program is missing, or you "
...
{
    const int BLOCKREAD_MAX_LINE_SIZE
        = 4120; // arbitrarily chosen to 4096 plus file-information-prefix
    ...
    if (lineEndChar == '\n') {
        ...
        while (!qProcess->canReadLine()) {
            // QProcess at least up to Qt 4.3.5 does not correctly reset error and errorString,
            // reset manually.
            qProcess->resetError();
            if (!qProcess->waitForReadyRead(100)) {
                if (!qProcess->hasError()) // waitForReadyRead returned false but reports no error
                {
                    // eighter the process is dead or the read channel has been closed -> abort read
                    // operation
                    return false;
                }
        ...
        *byteArray = QByteArray(buf);
        ...
        // when need to break _additionally_ to "\n" also at certain lineEndChar (for ex. at "\r" to
        // read progress-info of rsync) more tricky case, where lineEndChar isn't "\n" ->
        ...
        // qDebug() << "!readBuffer.contains(QChar(lineEndChar))=" <<
        // (!readBuffer.contains(QChar(lineEndChar))) << "!readBuffer.contains(\"\\n\")" <<
        // (!readBuffer.contains("\n")) << "!qProcess->canReadLine()" << (!qProcess->canReadLine())
        // << "BLOCKREAD_MAX_LINE_SIZE-readBuffer.length()>0" <<
        // (BLOCKREAD_MAX_LINE_SIZE-readBuffer.length()>0);
        while (!readBuffer.contains(QChar(lineEndChar)) && !readBuffer.contains("\n")
            ...
            if (!qProcess->waitForReadyRead(100)) {
                ...
                if (BLOCKREAD_MAX_LINE_SIZE - readBuffer.length() - 1 > 0) {
                    nCharsRead = qProcess->read(buf,
                                                BLOCKREAD_MAX_LINE_SIZE - readBuffer.length() - 1);
                    result = nCharsRead >= 0;
        ...
        if (qProcess->canReadLine()) {
            if (BLOCKREAD_MAX_LINE_SIZE - readBuffer.length() - 1 > 1) {
                nCharsRead = qProcess->readLine(buf,
                                                BLOCKREAD_MAX_LINE_SIZE - readBuffer.length() - 1);
                result = nCharsRead >= 0;
                readBuffer.append(QByteArray(buf, nCharsRead));
            }
        ...
        int pos = std::min((readBuffer.indexOf(QChar(lineEndChar)) + (2 * BLOCKREAD_MAX_LINE_SIZE))
                               % (2 * BLOCKREAD_MAX_LINE_SIZE),
                           (readBuffer.indexOf("\n") + (2 * BLOCKREAD_MAX_LINE_SIZE))
                               % (2 * BLOCKREAD_MAX_LINE_SIZE));
...
{
    if (qProcess == 0 || qProcess->state() == QProcess::NotRunning || killed)
        return false;
    if (abort) {
        QMutex mutex;
        mutex.lock();
